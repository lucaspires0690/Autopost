...
window.entrarCanal = function(channelId, channelName) { 
  ...
  document.getElementById('channel-management-title').textContent = `Gerenciando: ${escapeHtml(channelName)}`; 
...
window.excluirCanal = async function(channelId) { 
  ...
  try { 
    ...
    showError("Erro ao excluir canal."); 
  } 
}
...
function handleLogout() { 
  auth.signOut().catch(error => showError("Erro ao fazer logout.")); 
}
...
async function carregarCanais() { 
  ...
  try { 
    ...
    if (snapshot.empty) { 
      tableBody.innerHTML = '<tr><td colspan="5">Nenhum canal adicionado.</td></tr>'; 
      return; 
    } 
    ...
    snapshot.forEach(doc => { 
      ...
      const dataFormatada = canal.dataCriacao 
        ...
        : 'N/A'; 
      html += `<tr data-id="${escapeHtml(doc.id)}" data-nome="${nomeCanal}">
...
function solicitarAcessoYouTube() {
  if (!AppState.gisReady || !AppState.gapiReady) { 
    showError("Integração com Google não está pronta."); 
    return; 
  }
  
  AppState.tokenClient.callback = async (resp) => {
    if (resp.error !== undefined) { 
      showError("Erro na autorização com o Google."); 
      return; 
    }
    await buscarInfoCanalEAdicionar(resp);
  };
  
  // CRÍTICO: Solicita acesso offline para obter o refresh_token
...
async function buscarInfoCanalEAdicionar(tokenResponse) {
  try {
    ...
    if (response.result.items && response.result.items.length > 0) {
      ...
      const canalRef = db.collection('usuarios')
        .doc(AppState.currentUser.uid)
        .collection('canais')
        .doc(channelId);
      
      if ((await canalRef.get()).exists) { 
        showError(`Canal "${channelName}" já foi adicionado anteriormente.`); 
      ...
      if (tokenResponse.refresh_token) {
        ...
        console.log("✅ Refresh token obtido e será salvo!");
      } else {
        console.warn("⚠️ Refresh token NÃO foi retornado pelo Google. Pode ser necessário revogar acesso e reconectar.");
      }
      ...
      showError("Nenhum canal do YouTube encontrado na sua conta."); 
    }
  } catch (error) { 
    console.error("Erro ao adicionar canal:", error);
    showError("Ocorreu um erro ao adicionar o canal. Tente novamente."); 
  } finally { 
...
function handleFileUpload(event) { 
  ...
  Array.from(files).forEach(file => { 
    if (!validarArquivo(file, tiposAceitos)) { 
      showError(`Arquivo "${file.name}" é inválido.`); 
      return; 
    } 
    ...
    uploadTask.on('state_changed', 
      ...
      (error) => showError(`Erro ao enviar ${file.name}.`), 
      () => { 
...
function baixarModeloCSV() { 
  ...
  const exemplo = "video_exemplo.mp4,thumb_exemplo.jpg,Meu Título,Descrição do vídeo.,tag1;tag2,2025-12-31,18:00"; 
  const csv = `${header}\n${exemplo}`; 
    ...
    complete: async (results) => {
      try {
        const linhasValidas = results.data.filter(row => 
          row.data_publicacao && row.hora_publicacao
        );
        
        if (linhasValidas.length === 0) { 
          logCsvStatus('Nenhuma linha válida encontrada no arquivo.', 'error'); 
          return; 
        }
        
        logCsvStatus(`${linhasValidas.length} linhas válidas encontradas. Salvando...`);
        ...
        logCsvStatus(`ERRO: ${error.message}`, 'error'); 
      }
    },
    error: (error) => {
...
async function renderizarAgendamentos() {
  ...
  try {
    ...
    if (AppState.agendamentosCache.length === 0) { 
      tableBody.innerHTML = '<tr><td colspan="4">Nenhum agendamento encontrado.</td></tr>'; 
      return; 
    }
    ...
    AppState.agendamentosCache.forEach(agendamento => {
      ...
      if (agendamento.status === 'Publicado') statusClass = 'published';
      else if (agendamento.status === 'Erro') statusClass = 'error';
      else if (agendamento.status === 'Processando') statusClass = 'processing';
      
      html += `<tr>
        <td>${escapeHtml(agendamento.titulo)}</td>
        <td>${dataFormatada}</td>
        <td><span class="status-badge ${statusClass}">${escapeHtml(agendamento.status)}</span></td>
        <td class="actions">
...
async function limparTodosAgendamentos() {
  if (!AppState.canalAtual || !confirm("Tem certeza que deseja limpar TODOS os agendamentos?")) return;
  ...
  try {
    ...
    showSuccess(`${snapshot.size} agendamentos excluídos com sucesso.`);
    renderizarAgendamentos();
  } catch (error) { 
